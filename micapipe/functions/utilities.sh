...
#
# Utilities: environment variables & subfunctions used by all processing modules
#
...
# Directory with the resampled freesurfer surfaces
  ...
  export subject_dir=$out/${subject}/${SES}     					# Output derivative data directory
  export subject_bids=${BIDS}/${subject}/${SES} 					# Input BIDS raw data directory
...
# Diffusion directories derivatives
  export proc_dwi=$subject_dir/proc_dwi               		 			 # DWI processing directory
#  export proc_dwi=${subject_dir}/proc_dwi/test               									# CHANGED TO SUB IN DEGIBBSED DWI
  export dir_noddi=${proc_dwi}/noddi
  export dir_commit=${proc_dwi}/commit

  export proc_sc=${proc_dwi}/sc
  export dir_connectome=${proc_sc}/connectomes
  export dir_scWeights=${proc_sc}/weights

# Other derivative directories
...
# post structural Files (the resolution might vary depending on the dataset)
  if [ -f "${proc_struct}"/"${id}"_t1w_*mm_nativepro.nii.gz ]; then
      ...
      export T1fast_org=$proc_struct/first/${id}_t1w_*mm_nativepro_all_fast_origsegs.nii.gz
...
# Resting state proc files
  ...
  export dwi_reverse=$(ls "${subject_bids}/dwi/${subject}_${SES}"*-PA*dwi.nii* 2>/dev/null)
}
